...
struct Option
{
    Option() : param(ffm_get_default_param()), nr_folds(1), do_cv(false), on_disk(false), with_io1(false),with_io2(false) {}
    string tr_path, va_path, model_path, test_path1, out_path1, test_path2, out_path2;
    ffm_parameter param;
    ffm_int nr_folds;
    bool do_cv, on_disk, with_io1, with_io2;
};
...
void predict(string in_path,string out_path,ffm_model* model){

    int const kMaxLineSize = 1000000;

    FILE *f_in = fopen(in_path.c_str(), "r");
    ofstream f_out(out_path);
...
Option parse_option(int argc, char **argv)
{
    ...
    for(; i < argc; i++)
    {
        ...
        else if(args[i].compare("-i1") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify path after -i");
            i++;
            opt.test_path1 = args[i];
            opt.with_io1=true;
        }
        else if(args[i].compare("-o1") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify path after -i");
            i++;
            opt.out_path1 = args[i];
        }
        else if(args[i].compare("-i2") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify path after -i");
            i++;
            opt.test_path2 = args[i];
            opt.with_io2=true;
        }
        else if(args[i].compare("-o2") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify path after -i");
            i++;
            opt.out_path2 = args[i];
        }
...
int train(Option opt)
{
    ...
    else
    {
        ...
        if(opt.with_io1){
            predict(opt.test_path1,opt.out_path1,model);
        }
        if(opt.with_io2){
            predict(opt.test_path2,opt.out_path2,model);
        }
